...
> **Исправления (импорты):** все папки `labX/` стали Python-пакетами (`__init__.py`),
> импорты `common`/`server`/`client` → `lab1.common`/`lab1.server`/`lab1.client`,
> все команды запускаются из корня проекта (`python -m labX...`).
