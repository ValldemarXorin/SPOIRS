    ...
    def _do_upload(self, path, filename, size, offset, use_udp):
        ...
        if use_udp:
            ...
            if not resp or "READY" not in resp:
                print(f"Server not ready: {resp}"); return False
        else:
    ...
    def _do_download(self, filename, offset, use_udp):
        ...
        if use_udp:
            resp_str = RUDPSocket(self.udp_socket,
                                  (self.host, self.port)).send_command(cmd)
            if not resp_str:
                print("No UDP response"); return False
        else:
            if not self.connected: return False
            send_all(self.tcp_socket, (cmd + "\n").encode())
            r = recv_until(self.tcp_socket, COMMAND_TERMINATOR)
        ...
        if "ERROR" in resp_str:
            print(resp_str); return False
        fsize = self._parse_resp(resp_str)
        if fsize <= 0:
            print(f"Bad response: {resp_str}"); return False
        ...
        if use_udp:
            ...
            try:
                # dest_addr=None — recv_stream примет DATA с любого addr
                # (download-поток на сервере шлёт с нового порта)
        ...
        if received != fsize:
            print("Incomplete transfer"); return False
        return True
            ...
            with open(fp, "ab" if offset > 0 else "wb") as f:
                while rcv < size:
                    data = recv_exact(self.tcp_socket,
                                      min(BUFFER_SIZE, size - rcv), timeout=60)
                    if data is None:
                        print("\nConnection lost"); break
                    f.write(data); rcv += len(data)
                    self._prog(rcv, size)
        except IOError as e:
